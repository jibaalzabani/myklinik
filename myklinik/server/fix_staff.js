...
db.connect(err => {
    ...
    console.log("Database Terhubung. Sedang memperbaiki akun Staff...");

    // Update password staff menjadi 123456 yang valid
    const sql = `UPDATE users SET password = ? WHERE email = 'staff@klinik.com'`;
    ...
    db.query(sql, [hashBaru], (err, result) => {
        if (err) {
            ...
            console.log("========================================");
            console.log("SUKSES! Password Staff sudah di-reset.");
            console.log("Silakan Login dengan:");
            console.log("Email: staff@klinik.com");
